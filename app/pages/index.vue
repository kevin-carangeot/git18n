...
watch(selectedPull, async (newBranch) => {
    ...
    try {
        ...
        diffData.value = await $fetch('/api/pr-diff', { query: { branch: newBranch, path: filePath } });
    } catch (err) {
...
const createPullRequest = async () => {
    ...
    const getRepoInfo = () => {
        const match = (config.public.githubRepoUrl as string).match(/github\.com\/([^\/]+)\/([^\/]+)/);
    ...
    try {
        ...
        if (Object.keys(translationsPayload).length === 0) throw new Error("No translations to save.");
        ...
        toast.add({ title: 'PR Created!', color: 'green', icon: 'i-heroicons-check-badge', timeout: 5000, callback: () => window.open(response.url, '_blank') });
...
<template>
    <UContainer class="max-w-4xl py-6">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <UButton v-if="hasResults" icon="i-heroicons-arrow-left" color="gray" variant="ghost" label="Back to Config" @click="resetView" />
        </div>

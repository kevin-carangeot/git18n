...
const computeDiffHtml = (oldText: string, newText: string) => {
    // Utilise diffWordsWithSpace pour ne pas couper les mots en deux
    ...
    return diff.map(part => {
        ...
        // Suppression (Rouge barré) - Important de l'afficher pour le contexte
        if (part.removed) {
...
<template>
    <div class="font-mono text-[13px] leading-7 whitespace-pre">
        <div v-for="(value, key) in data" :key="key">
            ...
            <div v-if="typeof value === 'object' && !value.status" class="pl-4 border-l border-gray-200 dark:border-gray-800">
            ...
            <div v-else-if="value.status === 'added'" class="pl-4 border-l-2 border-green-500 bg-green-50/50 dark:bg-green-900/10 py-1">
                <span class="text-green-700 dark:text-green-500 font-bold">"{{ key }}"</span>: <span class="text-green-600">"{{ value.val }}",</span>
            </div>

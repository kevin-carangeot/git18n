...
All GitHub HTTP access is centralized in `server/services/github.ts` (`createGitHubClient`): `listPulls`, `getRepo`, `getFileRaw`, `getBranchSha`, `getFile`, `putFile`, `createPullRequest`. Every route goes through it (no hand-rolled `fetch`), so headers/`User-Agent` stay consistent. Reads return `null` only on a genuine 404 — any other error propagates instead of being silently treated as an empty file.

Server utils are pure functions worth knowing:

- `diff.ts` — `calculateDetailedDiff` is the single source of truth: it walks both objects once and returns a `DiffTree` whose leaves carry `status`/`old`/`new`/`val`. An added nested group is recursed into so each leaf is tagged individually (keeps the tree renderable). `flattenDiffTree` derives the plain key→value object to translate, and `countLeaves` counts the changed leaves — both from that tree.
...
`app/composables/useTranslationFlow.ts` owns the PR → diff → translate → create-PR flow: state (`selectedPull`, `diffData`, loading flags, `editableTranslations`) plus the actions. On PR selection it fetches the diff; `startTranslation` translates every `targetLanguages` entry concurrently via `/api/translate` (`Promise.allSettled`, so one language failing doesn't abort the rest); `createPullRequest` validates the edited JSON and posts to `/api/create-pr`; `resetView` (the "Translate another PR" button) clears the selected PR, diff and results to start over. `app/pages/index.vue` just wires this composable to the UI (PR list + result tabs). When not configured it shows `ConfigWizard`; `/settings` shows the same config via `ConfigForm`.
...
- Import aliases: `~/` → `app/`, `~~/` → project root (server uses `~~/server/...`). Code shared between client and server lives in `shared/` (e.g. `~~/shared/utils/locale-path`, `~~/shared/types/json` for the common `JsonObject` type).
